...
> **阶段 1～2「亲自懂」精讲**：见 [[阶段1-2亲自懂]]（心智模型、端口/Token/事件、对照本仓库路径与自测题）。
...
| **4** | Rust 速成 + Tauri 2 套壳             | 3~4 周 | 同套 Vue 3：Web 仍可用 + Win/Mac 安装包能开会 | Rust 读懂级、Tauri 2 套壳、双分发与系统权限         |
...
2. 开两个浏览器窗口（各生成一个不同 `--identity` 的 Token）→ 互相看到听到；再视情况换局域网另一台设备。
3. 玩坏它：关容器看页面报什么错、Token 过期是什么现象、`--dev` 日志里找到「有人进房、有轨发布」对应的行。
...
# 2) HotGo（在 server/backend，Air 热更新仅起 HTTP）
...
1. **HotGo** `conference` **addon**（`server/backend/addons/conference`）：
...
2. **独立 Vue3 会议页**（`client/`，`livekit-client` + Ant Design Vue）：
  ...
  - 阶段 4 在同目录加 `src-tauri/` 套壳；**一套 Vue、Web + 桌面两种分发**（不拆两套前端）
...
> **状态：已完成**（2026-07-28，双网页自测验收）。



### 做什么（按优先级）

1. [x] **成员列表** + 主持人能力：踢人、全员静音（LiveKit Server API，走 Go）；空房/只剩一人可 `claimHost` 接任
2. [x] **文字聊天**：LiveKit 的 data channel（`publishData`），不用额外起 WebSocket 服务
3. [x] **设备选择**：麦克风/摄像头/扬声器下拉切换（设备兼容是泥潭，早暴露）
4. [x] **布局**：无人出画面 → 头像墙；有人开摄像头或投屏 → 自动演讲者布局；侧栏点击切换主视图（为多人共享预留）
5. [x] **网络质量指示**：订阅 `ConnectionQuality` 事件，画个信号格
6. [x] 界面用 Ant Design Vue 整体过一遍，能看即可，别陷进像素级调整

补充落地（打磨过程中）：

- [x] 进房前可选麦/摄像头，**默认静音、关摄像头**
- [x] 关摄像头时主区显示昵称全名（避免黑屏）
- [x] 投屏清晰度目标 2K@60（浏览器实际采集可能达不到）；页面内按钮控制开/停共享（浏览器系统条无法去掉）
- [x] 开发期 Vite `host: 0.0.0.0` 可用内网 IP 打开页面；**真机局域网开会仍需 https + LiveKit 宣告内网 IP**（`http://内网IP` 非安全上下文）



### 验收标准（本阶段核心就这一条）

- [x] **验收改为双网页自测**：两个浏览器标签/窗口（不同昵称进同一房间）互测通过
- [ ] （可选补做）拉 2~3 个真人开一次 ≥30 分钟真实会议，问题记成 issue 清单
...
- 主持人：按房间 Redis 缓存 identity 判定（与昵称无关）；空房可重新抢；进房后可认领
...
- 浏览器原生「正在共享屏幕」小条无法用页面按钮替代（安全策略）
- 用内网 IP 的 http 打开页面 ≠ 能稳定开会（安全上下文 + ICE 地址）
...
> 桌面端和 Rust 是你的双空白区，但此时你手里已有一个**跑通的 Vue 3 会议页**，Tauri 只是给它换个壳——这是全路线里刻意安排的最低风险切入点。
>
> **状态：已完成（最小 WVP）**（2026-07-28）。打包与换地址见 [[操作手册-最小WVP]]。
>
> **技术栈钉死（2026-07-28）**：
> - **前端**：Vue 3（现网已是 `vue@^3.5` + Vite + Ant Design Vue），不是 Vue 2
> - **壳**：**Tauri 2**（当前稳定最新线；crates.io 上 `tauri` 约 `2.11.x`）。脚手架时用官方最新 2.x，**不要**再起 Tauri 1
> - **形态**：一套 Vue 3 前端，两种分发（Web + 桌面）；**不**拆 `client/web` + `client/tauri` 两套工程
...
```
client/                 # 唯一会议前端（阶段 2~3 已有）
  src/                  # Join / Room / LiveKit / 会控……Web 与桌面共用
  src-tauri/            # 本阶段新增：Rust 壳、系统权限、托盘、打包
  package.json          # 脚本区分两种模式
```

| 模式 | 命令（示意） | 产物 / 用途 |
| ---- | ------------ | ----------- |
| **Web** | `pnpm dev` / `pnpm build` | 浏览器打开；零安装、可嵌门户 / 交付包里的静态页（阶段 5 仍要） |
| **桌面** | `pnpm tauri dev` / `pnpm tauri build` | `.dmg` / `.msi`；政企装机、权限引导、托盘常驻 |

桌面特有能力（托盘、开机自启、本地配置、权限引导、日后 updater）用 `@tauri-apps/api` / `__TAURI__` 分支；会议逻辑 **只维护一份**。

**明确不做（本阶段）**：

- ❌ 拆 `client/web` + `client/desktop` 两套 Vue（会立刻 drift + monorepo 成本）
- ❌ 为桌面重写会议 UI 或换 LiveKit SDK
- ❌ 上 LiveKit Rust 原生媒体管线（仍属阶段 6「有证据再动」）

真要拆共享包，等阶段 5/6 出现「桌面必须原生推流 / Web 必须极简嵌入」硬分歧再说。
...
学法：rustlings 挑前三分之一做做找手感 → 让 Agent 把 **已接入的** `src-tauri/` **逐文件讲解**给你听 → 之后在真实项目里遇到再问。



### 5.2 Tauri 工程（2~3 周）

1. **就地套壳**：在现有 `client/`（Vue 3 + Vite）接入 **Tauri 2 最新稳定版**（`src-tauri/`），`tauri.conf.json` 的前端指向当前 Vite 工程；**不要**另起空模板再搬代码，**不要**用 Tauri 1
2. **双模式脚本**：`dev` / `build` 继续只跑网页；新增 `tauri:dev` / `tauri:build`（或官方 `tauri` 脚本）跑壳；验收时两种都要能进同一 HotGo + LiveKit
3. **系统能力按需加**（先能开会，再锦上添花）：窗口控制 → 单实例锁 → 系统托盘 → 本地配置（设备偏好等）→ 开机自启（可后置）
4. **平台权限——本阶段最大的工作量**：
  - macOS：`Info.plist` 加 `NSCameraUsageDescription` / `NSMicrophoneUsageDescription`；屏幕录制要用户去系统设置勾选 → 做**引导页**（阶段 3 记下的浏览器权限坑，在这里换壳再过一遍）
  - Windows：确认 WebView2 策略（Win10+ 一般自带；老环境评估离线安装器）
5. **打包**：`tauri build` 出 `.dmg` / `.msi`；代码签名自用阶段可欠，发布前再补
6. **Web 不丢**：套壳完成后，`pnpm build` 出的静态页仍要能单独开会（证明共用前端没被壳绑死）



### 验收标准

- [x] **Web 与桌面两条路径都能进会**：浏览器 + 安装包各至少一次，同一房间互通音视频
- [ ] 两台真机（一台 Win 一台 Mac）安装桌面端、进会、屏幕共享全通（跨平台装机可补做）
- [x] 能回答：某个报错是 WebView 里的前端崩了，还是 Rust 侧 panic 了？各自去哪看日志？（第一次问询里的「底线」）
- [x] 能说清：为什么会议逻辑只在 `client/src`，壳只在 `src-tauri`



### 你要亲自懂的

- Tauri = 系统 WebView（前端）+ Rust 进程（系统侧），`command` / 事件是两者的桥
- 「一套前端、两种分发」：Web 是 Vite 产物，桌面是 WebView 加载同一套 UI
- `tauri.conf.json` 与 capabilities/权限声明的结构
- 编译期错误（cargo 拦下的）和运行时 panic 的区别
- 出问题分层：Vue/LiveKit（`client/src`）→ 壳/权限（`src-tauri`）→ Token/HotGo → LiveKit 服务端



### 坑预警（提前认账，别到时候怀疑人生）

- 设备兼容问题的工作量 ≈ 界面开发本身，预留缓冲
- 桌面端权限引导做不好，用户会觉得「装了客户端反而不如网页」——引导页是产品体验，不是可选装饰
- 如果目标客户有 Win7 级老内网环境，Tauri 的 WebView2 依赖是硬伤，届时评估 Electron 兜底——**阶段 4 结束时做一次这个决策**，别拖到交付前
- 别为了「目录好看」提前拆 monorepo；拆包成本远高于现在多写几个 `if (isTauri)`
---
...
6. 客户端补齐（仍挂在同一 `client/`）：自动更新（Tauri updater）、崩溃日志落盘；Web 静态页进交付包
...
| 3   | 2026-07-28 | 2026-07-28 | 半天   | 会控/聊天/设备/自动布局/网络质量；双网页自测通过；真人 30 分钟会可选补做；业务会议室/大厅留给阶段 5 |
| 4   | 2026-07-28 | 2026-07-28 | 当天   | Vue 3 + Tauri 2 最小壳；安装包能开会；操作见 [[操作手册-最小WVP]]；托盘/引导页/跨 Win 真机可后补 |
| 5   |            |            |      |                                                                            |
| 6   |            |            |      |                                                                            |
